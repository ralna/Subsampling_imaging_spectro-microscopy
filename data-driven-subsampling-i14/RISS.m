function [energiesToSample, spatialRowsToSample, energyIndices] = RISS(p, spectrumDictionary, energies, nX, nY)
% Input:
...
%                       taken
% spatialRowsToSample = the matrix of row indices to sample at all other
%                       energies
...
% Matrix of rows to sample for each energy level - the first column is the
% energy levels, the next columns is the spatial row indices to be sampled.
% The energy levels for which we have done full scans already are kept
% empty.
spatialRowsToSample = zeros(nE, numberOfRows + 1);
spatialRowsToSample(:,1) = energies;

for jj = 1:nE
    if ~ismember(jj, energyIndices)
        spatialRowsToSample(jj, 2:end) = sort(ARP(Uk));     
    end
end
